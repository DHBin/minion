 ...
 * @date 2020/3/24
 */
public class MinionUserAuthenticationConverter implements UserAuthenticationConverter {
    ...
    public Authentication extractAuthentication(Map<String, ?> map) {
        if (map.containsKey(USERNAME)) {
            ...
            Object id = map.get(Constant.USER_ID_KEY);
            if (userDetailsService != null) {
                UserDetails user = userDetailsService.loadUserByUsername((String) map.get(USERNAME));
                authorities = user.getAuthorities();
                principal = user;
            } else {
                principal = new MinionUser(id, String.valueOf(principal), "N/A", authorities);
            }
